...
// Gerçekçi kıtalar ve ülkeler
function defineContinentsAndCountries() {
    // Kıtalar (net ayrım)
    // Kuzey Amerika
    for (let y = 10; y < 60; y++) for (let x = 20; x < 90; x++) {
        if (y < 20 && x > 80) continue; // Kanada daraltma
        if (y > 50 && x < 30) continue; // Meksika sınırı
        if (x > 85 && y > 40) continue; // Atlantik ayrımı
        setPixel(x, y, false, "North America", null);
    }
    // Güney Amerika
    for (let y = 60; y < 120; y++) for (let x = 50; x < 100; x++) {
        if (y > 100 && (x < 60 || x > 90)) continue; // Konik şekil
        if (x < 55 && y < 70) continue; // Karayip ayrımı
        setPixel(x, y, false, "South America", null);
    }
    // Avrupa
    for (let y = 10; y < 50; y++) for (let x = 110; x < 150; x++) {
        if (y > 40 && x > 140) continue; // Daraltma
        if (x < 115 && y > 30) continue; // Atlantik ayrımı
        setPixel(x, y, false, "Europe", null);
    }
    // Afrika
    for (let y = 50; y < 110; y++) for (let x = 100; x < 160; x++) {
        if (y > 90 && (x < 110 || x > 150)) continue; // Üçgenimsi
        if (x > 145 && y > 80) continue; // Madagaskar ayrımı
        setPixel(x, y, false, "Africa", null);
    }
    // Asya
    for (let y = 10; y < 90; y++) for (let x = 160; x < 280; x++) {
        if (y > 70 && x > 260) continue; // Japonya/Sibirya
        if (x > 270 && y < 30) continue; // Pasifik ayrımı
        setPixel(x, y, false, "Asia", null);
    }
    // Avustralya
    for (let y = 110; y < 140; y++) for (let x = 200; x < 240; x++) {
        if (x < 205 && y < 115) continue; // Hint Okyanusu ayrımı
    ...
    const countries = [
        ...
        { name: "Kurdistan", continent: "Asia", positions: [[155,45],[156,45],[155,46],[156,46],[157,45],[157,46],[158,45],[158,46],[159,45],[159,46]], features: { health: 3, education: 3, economy: 2 }, flag: "images/kurdistan.svg" },
        ...
        { name: "Syria (Kurdistan)", continent: "Asia", positions: [[150,47],[151,47],[150,48],[151,48],[152,47],[152,48],[153,47],[153,48],[154,47],[154,48]], features: { health: 3, education: 3, economy: 2 }, flag: "images/kurdistan.svg" },
...
if (window.Telegram?.WebApp) {
    ...
    window.Telegram.WebApp.MainButton.setText('Buy via @tribute').show().onClick(() => buyPixel());
}
...
// Satın alma
function buyPixel() {
    window.open('https://tribute.app/@your-tribute-account', '_blank');
}
